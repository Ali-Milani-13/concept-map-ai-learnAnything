...
// 1. Initialize Groq to use your Cloudflare Worker
const groq = new Groq({
...
const systemInstruction = `
    ...
    // 2. Call the AI and EXPLICITLY choose the model here
const chatCompletion = await groq.chat.completions.create({
      messages: [
        { role: 'system', content: systemInstruction },
        { role: 'user', content: userPrompt }
      ],
      model: 'openai/gpt-oss-120b', // <--- Forcing the new OpenAI model
      temperature: 0.2,
      reasoning_effort: 'medium', // <--- Forcing it to think deeper
